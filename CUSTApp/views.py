...
class TemplateDisableAPIView(generics.DestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        instance.status = 0
        instance.save()
        return Response({"message": "Application disabled successfully"}, status=status.HTTP_200_OK)
...
class RequestCreate(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        request_serializer = RequestSerializer(data={
    ...
    def get(self, request, request_id):
        try:
            ...
            if application.status != 1:
                return Response({"error": "Application is disabled"}, status=status.HTTP_400_BAD_REQUEST)

            template = application.application_desc
            if not template:
                return Response({"error": "Template not found for this application"}, status=status.HTTP_404_NOT_FOUND)

            attributes = {
                attr.attribute_name.split('_', 2)[-1]: attr.attribute_value
                for attr in TemplateAttributes.objects.filter(attribute_name__startswith=f"request_{request_id}_")
            }
            attributes.update({
            ...
            elements.append(Paragraph("Capital University of Science and Technology", styles['Heading1']))
            ...
            elements.append(Spacer(1, 12))
            elements.append(Paragraph(f"{attributes['issuer_name']}\nDeputy Registrar", styles['Normal']))

            doc.build(elements)
            pdf_buffer.seek(0)

            return FileResponse(pdf_buffer, as_attachment=True, filename=f'{application.short_name.replace("_", " ")}_{student.uu_id}.pdf')
...
class GeneratePDFAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        user_data = {
            'users': {
                'student': {
                    ...
                    'father_name': user.father_name if user.user_type == 'student' else '',
                    'address': user.address if user.user_type == 'student' else '',
                    'program_name': user.program_name if user.user_type == 'student' else '',
                    ...
                    'designation': user.designation if user.user_type == 'student' else '',
                    'remark': user.remark if user.user_type == 'student' else '',
                    'phone_number': user.phone_number if user.user_type == 'student' else '',
                ...
                'staff': {
                    ...
                    'father_name': user.father_name if user.user_type == 'staff' else '',
                    'address': user.address if user.user_type == 'staff' else '',
                    'program_name': user.program_name if user.user_type == 'staff' else '',
                    ...
                    'designation': user.designation if user.user_type == 'staff' else '',
                    'remark': user.remark if user.user_type == 'staff' else '',
                    'phone_number': user.phone_number if user.user_type == 'staff' else '',
                    'cgpa': str(user.cgpa) if user.cgpa else '',
        ...
        rendered_content = re.sub(r'\[([^\]]+)\]', replace_placeholders, template_content)
...
class OTPSendView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        from_email = settings.EMAIL_HOST_USER if settings.EMAIL_HOST_USER else 'support@custapp.pk'
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if user.otp == otp:
                ...
                print(f"Generated Tokens - Access: {access_token}, Refresh: {refresh_token}")  # Log tokens
